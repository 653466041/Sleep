import { getTrackById } from '../data/audio'
...
const TIMER_MINUTES: Record<string, number> = {
  '15': 15,
  '30': 30,
  '60': 60,
}
...
function getCacheEntries(): AudioCacheEntry[] {
  try {
...
function setCacheEntries(entries: AudioCacheEntry[]) {
  try {
...
function removeSavedAudio(filePath: string) {
...
function pruneAudioCache(entries: AudioCacheEntry[]): AudioCacheEntry[] {
...
function getCachedTrackPath(track: { id: string; url: string }): string {
  const now = Date.now()
...
function saveCacheEntry(track: { id: string; url: string }, filePath: string) {
  const now = Date.now()
...
function cacheTrackAfterPlayback(track: { id: string; url: string }, callback: (filePath: string) => void) {
...
function scheduleSleepTimer(value: string) {
  clearSleepTimer()
...
export function playTrackById(id: string): boolean {
  ensureInitialized()
...
export function setSleepTimer(value: string): boolean {
  ensureInitialized()
